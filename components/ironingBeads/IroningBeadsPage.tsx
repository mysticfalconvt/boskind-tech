...
type ViewMode = 'projects' | 'design';

export const IroningBeadsPage: React.FC = () => {
  const { currentProject, projects } = useIroningBeadsStore();
  const [viewMode, setViewMode] = useState<ViewMode>('projects');
  const [isLoading, setIsLoading] = useState(true);

  // Initialize the component and determine initial view
  useEffect(() => {
    // Simulate loading time for better UX
    const timer = setTimeout(() => {
      setIsLoading(false);
      
      // If there's a current project, show the design view
      if (currentProject) {
        setViewMode('design');
      } else {
  ...
  // Update view mode when current project changes
  useEffect(() => {
      ...
      <div className="min-h-screen bg-base-200 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-base-content mb-2">
            Loading Ironing Beads Designer
          </h2>
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center max-w-md mx-auto p-6">
        ...
        <p className="text-gray-600 mb-4">
          {error.message || 'An unexpected error occurred while loading the designer.'}
        </p>
        <button
          onClick={() => window.location.reload()}
          className="btn btn-primary px-4 py-2 text-white rounded-lg hover:shadow-md transition-all duration-200"
        >
          Reload Page
        </button>
      </div>
